...
2. **Migration de schema dos embeddings (384 → 768 dims) — tentada em 2026-08-22, revertida no mesmo dia.** Aplicada com aprovação do usuário, mas o aviso dado antes de rodar subestimou o impacto: eu disse que "degradaria temporariamente a busca RAG do Railway"; na prática, `match_chunks` passou a exigir `vector(768)` e o Railway continua gerando a query embedding localmente em 384 dims — toda chamada virou erro de dimensão incompatível, não uma degradação. Isso derrubou com 500 em produção: geração de quiz (`atividades.py`), plano de estudos (`score.py`), e upload de texto digitado (`documentos.py::upload_text`, sem rollback — ficava documento órfão sem chunks). Upload de PDF também quebrava, mas tinha rollback seguro já existente.
   ...
   - Reprocessamento dos 361 chunks em 384 dims feito via rota temporária `POST /_maint/reembed-384` no próprio Railway (`backend/main.py`, protegida por header `x-backfill-token`, usa o `embed_texts` local de sempre — sem cota, sem API externa). Remover essa rota depois de confirmar `restantes: 0`.
   - **Lição para a próxima tentativa**: migrar o schema de embeddings só depois que o Railway parar de gerar query embeddings sozinho — ou trocando o Railway pra falar com o Gemini também (mas aí é reprocessar trabalho de um serviço que vai ser desligado), ou (melhor) só depois do flip de `NEXT_PUBLIC_API_URL` pra Edge Function, quando o Railway não estiver mais no caminho de nenhuma requisição real. Rodar o backfill do zero nessa janela, e verificar de antemão que a cota do Gemini aguenta ~361 chunks em uma sessão (ou espalhar ao longo de dias).
